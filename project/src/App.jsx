...
import { Question } from "./components/Question";
import { Results } from "./components/Results";
import { fetchData } from "./quizService";
...
function QuizContent() {
  ...
  const url = "https://api.jsonserve.com/Uw5CrX"; // API URL

  useEffect(() => {
    async function loadQuestions() {
      try {
        const fetchedQuestions = await fetchData(url);
  ...
  if (!state.isQuizStarted) {
    return <Instructions totalQuestions={quizQuestions.length} onStart={handleStart} />;
  }
  ...
  return <Results questions={quizQuestions} userAnswers={state.answers} score={state.score} onRestart={handleRestart} />;
}
